...
//0812 claude.ai V2 try fix the problems
...
function AppContent() {
  ...
  const currentConversationItemRef = useRef<string | null>(null);
  ...
  // 超強版文字提取函數
  function extractAssistantTextFromAnyEvent(ev: any): string {
    console.log("🔍 Extracting text from event:", ev?.type, ev);
    
    // 方法1: 直接從事件中提取
    if (ev?.delta && typeof ev.delta === "string") return ev.delta.trim();
    if (ev?.text && typeof ev.text === "string") return ev.text.trim();
    if (ev?.transcript && typeof ev.transcript === "string") return ev.transcript.trim();
    ...
    // 方法3: 從內容陣列提取
    const contentArray = r?.content || ev?.content || [];
    ...
    // 方法4: 從 output 陣列提取
    const outputs = r?.output || ev?.output || [];
  ...
  async function postLog(log: { role: "user" | "assistant" | "system"; content: string; eventId?: string }) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("✅ Log saved successfully:", result.key);
      } else {
        console.error("❌ Log API error:", response.status, response.statusText);
      }
    } catch (e) {
      console.error("❌ postLog failed:", e);
      ...
      try {
        navigator.sendBeacon("/api/logs", new Blob([body], { type: "application/json" }));
        console.log("📡 Fallback: Used sendBeacon");
      } catch (beaconError) {
  ...
  async function connectToRealtime() {
    ...
    try {
      ...
      dc.addEventListener("message", (e: MessageEvent) => {
        ...
        // 保持最近100個事件
        if (allEventsRef.current.length > 100) {
          allEventsRef.current.shift();
        }
        ...
        if (eventType === "conversation.item.created") {
          ...
          if (item?.role === "assistant") {
            currentConversationItemRef.current = item.id;
            console.log("🤖 Assistant conversation item created:", item.id);
          }
        }
        ...
        // —— 累積文字內容的各種事件 ——
        if (eventType.includes("delta") || eventType.includes("text")) {
          const deltaText = extractAssistantTextFromAnyEvent(eventData);
          if (deltaText) {
            assistantBufferRef.current += deltaText;
            console.log("📝 Text delta accumulated:", deltaText, "Buffer length:", assistantBufferRef.current.length);
          }
        }
        ...
        if (eventType === "response.content_part.added") {
          const part = eventData.part;
          console.log("➕ Content part added:", part?.type, part);
          if (part?.type === "text" && part?.text) {
            if (!assistantBufferRef.current.includes(part.text)) {
              assistantBufferRef.current += part.text;
              console.log("📝 Added content part text to buffer");
            }
          }
        }
        ...
        if (eventType === "response.content_part.done") {
          const part = eventData.part;
          console.log("✅ Content part done:", part?.type, part);
          if (part?.type === "text" && part?.text) {
            if (!assistantBufferRef.current.includes(part.text)) {
              assistantBufferRef.current += part.text;
              console.log("📝 Added final content part text to buffer");
            }
          }
        }
        ...
        if (eventType === "response.done" || 
          ...
          console.log("🏁 Assistant response completed. Event:", eventType);
          ...
          if (assistantBufferRef.current.trim()) {
            assistantText = assistantBufferRef.current.trim();
            console.log("✅ Strategy 1: Used accumulated buffer");
          }
          
          // 策略2: 從當前事件提取
          if (!assistantText) {
            assistantText = extractAssistantTextFromAnyEvent(eventData);
            if (assistantText) console.log("✅ Strategy 2: Extracted from current event");
          }
          
          // 策略3: 從最近的相關事件中搜尋
          if (!assistantText) {
            const recentEvents = allEventsRef.current.slice(-20); // 最近20個事件
            for (const recentEvent of recentEvents.reverse()) {
              if (recentEvent.type.includes("text") || recentEvent.type.includes("content")) {
                const eventText = extractAssistantTextFromAnyEvent(recentEvent.data);
                if (eventText) {
                  assistantText = eventText;
                  console.log("✅ Strategy 3: Found text in recent event:", recentEvent.type);
                  break;
                }
              }
            }
          ...
          console.log("📋 Final assistant text:", assistantText || "[NO TEXT FOUND]");
          console.log("📊 Recent events:", allEventsRef.current.slice(-5).map(e => e.type));
          
          if (assistantText) {
            postLog({
              role: "assistant",
              content: assistantText,
              eventId: currentResponseIdRef.current || eventData.response?.id || eventData.id || eventData.item_id,
            });
          } else {
            console.warn("⚠️ No assistant text found! Full event:", eventData);
            console.warn("⚠️ Current buffer:", assistantBufferRef.current);
            console.warn("⚠️ All recent events:", allEventsRef.current.slice(-10));
          }
          
          // 重置狀態
          assistantBufferRef.current = "";
          currentResponseIdRef.current = null;
          currentConversationItemRef.current = null;
        }
      });
  ...
  const handleTalkButtonDown = () => {
    if (sessionStatus !== "CONNECTED" || dataChannel?.readyState !== "open") return;
    console.log("🎙️ PTT button pressed down");
    cancelAssistantSpeech();
    setIsPTTUserSpeaking(true);
    setIsListening(true);
    sendClientEvent({ type: "input_audio_buffer.clear" }, "clear PTT buffer");
  ...
  const handleTalkButtonUp = () => {
    if (sessionStatus !== "CONNECTED" || dataChannel?.readyState !== "open" || !isPTTUserSpeaking) return;
    console.log("🎙️ PTT button released");
    setIsPTTUserSpeaking(false);
    setIsListening(false);
    sendClientEvent({ type: "input_audio_buffer.commit" }, "commit PTT");
    sendClientEvent({ type: "response.create" }, "trigger response PTT");
    ...
    <div className="text-base flex flex-col bg-gray-100 text-gray-800 relative" style={{ height: "100dvh", maxHeight: "100dvh" }}>
      <div className="p-3 sm:p-5 text-lg font-semibold flex justify-between items-center flex-shrink-0 border-b border-gray-200">
        <div className="flex items-center cursor-pointer" onClick={() => window.location.reload()}>
          ...
          <div>AI 營養師 (Debug Mode)</div>
        </div>

        <div className="flex items-center gap-3">
          {/* 除錯資訊顯示 */}
          <div className="text-xs text-gray-600">
            Events: {allEventsRef.current.length} | 
            Buffer: {assistantBufferRef.current.length}chars
          </div>
